...
// RAZORPAY ROUTE DRIVER PAYOUT ACCOUNT ENDPOINTS
...
router.post('/create-order', async (req, res) => {
  await releaseExpiredBookings().catch(e => console.error('[CLEANUP] Pre-order sweep error:', e));
...
// OBSOLETE WALLET ENDPOINTS (DEPRECATED - PullUp uses Razorpay Route Direct Payouts)
router.post('/refresh-wallet', async (_req, res) => {
  res.status(410).json({ success: false, message: 'Internal wallet is deprecated. Earnings are paid directly to your Razorpay Linked Account.' });
});
router.post('/verify-upi', async (_req, res) => {
  res.status(410).json({ success: false, message: 'UPI verification is deprecated. Please set up your Razorpay Linked Account.' });
});
router.post('/request-withdrawal', async (_req, res) => {
  res.status(410).json({ success: false, message: 'Manual withdrawals are deprecated. Payouts are transferred automatically via Razorpay Route.' });
});
router.post('/approve-withdrawal', async (_req, res) => {
  res.status(410).json({ success: false, message: 'Manual withdrawal approval is deprecated. All payouts use Razorpay Route.' });
});
